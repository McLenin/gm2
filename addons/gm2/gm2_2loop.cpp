   ...
   for(int k=0; k<2; k++) {
      result += ( (AAC_(k) * F1C(x__k(k)) / 12.
                  ...
                  - (0.5 * AAC_(k) * F1C(x__k(k))
                    ...
                    * log(sqr(MSv(1) / mu_DREG)) );
   }

   return  sqr(model.get_EL0()) * sqr(oneOver16PiSqr) * sqr(MM / MSv(1)) * result;
}
...
Eigen::Matrix<std::complex<double>,3,3> lambda_mu_cha(const MSSM_gm2_wrapper& model) {
...
Eigen::Matrix<std::complex<double>,2,2> lambda_stop(const MSSM_gm2_wrapper& model) {
...
Eigen::Matrix<std::complex<double>,2,2> lambda_sbot(const MSSM_gm2_wrapper& model) {
   Eigen::Matrix<std::complex<double>,2,2> result;
   double TB(model.get_TB());
   double CB = 1. / sqrt(1. + sqr(TB));
   double TA(tan_alpha(model));
...
Eigen::Matrix<std::complex<double>,2,2> lambda_stau(const MSSM_gm2_wrapper& model) {
   Eigen::Matrix<std::complex<double>,2,2> result;
   double TB(model.get_TB());
   double CB = 1. / sqrt(1. + sqr(TB));
   double TA(tan_alpha(model));
...
double amua2LCha(const MSSM_gm2_wrapper& model) {
   double result = 0.;
   double MM(model.get_MM());
   double MW(model.get_MW());
   double MA0(model.get_MA0());
