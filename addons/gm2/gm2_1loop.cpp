...
Eigen::Matrix<std::complex<double>,4,2> n_L(const MSSM_gm2_wrapper& model) {
...
Eigen::Matrix<std::complex<double>,4,2> n_R(const MSSM_gm2_wrapper& model) {
   Eigen::Matrix<std::complex<double>,4,2> result;
   double gY(model.get_gY());
   double ymu(model.get_Ye()(1, 1));
   Eigen::Matrix<std::complex<double>,4,4> ZN(model.get_ZN());
...
Eigen::Array<std::complex<double>,2,1> c_L(const MSSM_gm2_wrapper& model) {
...
Eigen::Array<std::complex<double>,2,1> c_R(const MSSM_gm2_wrapper& model) {
   Eigen::Array<std::complex<double>,2,1> result;
   double ymu = model.get_Ye()(1, 1);
...
Eigen::Array<double,2,1> AAC(const MSSM_gm2_wrapper& model) {
...
Eigen::Matrix<double,4,2> AAN(const MSSM_gm2_wrapper& model) {
...
Eigen::Array<double,2,1> BBC(const MSSM_gm2_wrapper& model) {
   Eigen::Array<double,2,1> result;
   Eigen::Array<std::complex<double>,2,1> c__L(c_L(model));
   Eigen::Array<std::complex<double>,2,1> c__R(c_R(model));
   for(int k=0; k<2; ++k) {
...
Eigen::Matrix<double,4,2> BBN(const MSSM_gm2_wrapper& model) {
   Eigen::Matrix<double,4,2> result;
   Eigen::Matrix<std::complex<double>,4,2> n__L = n_L(model);
   Eigen::Matrix<std::complex<double>,4,2> n__R = n_R(model);
   for(int i=0; i<4; ++i) {
...
Eigen::Matrix<double,4,2> x_im(const MSSM_gm2_wrapper& model) {
   Eigen::Matrix<double,4,2> result;
   Eigen::Array<double,2,1> m_smu(model.get_MSmu());
   Eigen::Matrix<double,2,2> u_smu(model.get_USmu());
...
Eigen::Array<double,2,1> x_k(const MSSM_gm2_wrapper& model) {
   Eigen::Array<double,2,1> result;
   for(int k=0; k<2; ++k) {
      result(k) = sqr(model.get_MCha()(k) / model.get_MSv()(1)); // !!!
   }
...
double amuChipm(const MSSM_gm2_wrapper& model) {
   ...
   Eigen::Array<double,3,1> MSv(model.get_MSv());
